...
def sort_items(lst, is_item_completed):
    lst = sorted(lst, key=lambda t: t['title'].lower())
    incomplete_items = [item for item in lst if not is_item_completed(item)]
    complete_items = [item for item in lst if is_item_completed(item)]
